...
from model.normal.cxrmodel import CxrModel
from model.ops import base_ops
from model.ops import attention
from model.ops import utils


class SeqAttenModel(CxrModel):
    def __init__(self,
                 train_config,
                 model_config,
                 model_columns,
                 model_name="CTR", model_idx=0):
        super(SeqAttenModel, self).__init__(train_config, model_config, model_columns, model_name, model_idx)
    ...
    def user_seq_layer(self, user_input_layer, query_input_layer, user_seq_columns, fg, features, share_name=None):
        ...
        with tf.variable_scope(
            ...
            seq_feas = fg.sequence_features(features, self.mc.sequence_columns, fg.seq_length, fg.sequence_name, False)
            user_seq_layer = layers.input_from_feature_columns(seq_feas, user_seq_columns, scope=scope)
        ...
        item_sequence_mask = tf.not_equal(tf.sparse_tensor_to_dense(seq_feas.items()[0][1], "0"), "0")
        with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.attention_l2_reg)):
            ...
            with tf.variable_scope(name_or_scope=sc,
                ...
                items = tf.split(user_seq_layer, 10, axis=0)  # [?, f_num*f_embedding]*30
    ...
    '''
    def summary(self):
        super(SeqAttenModel, self).summary()
        '''
