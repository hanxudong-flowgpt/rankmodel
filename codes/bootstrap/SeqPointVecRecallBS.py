...
class Train(BootStrap):
    ...
    def buildGraph(self, task_id, context):
        ...
        for modelconf in jobconf["sources"]:
            ...
            # Build Graph
            model.build_main_part([batch_data, self.context_serving_ph], self.is_training)
    ...
    def createSession(self, target, task_id):
        ...
        saved_model_input = {'context': self.models[0].context_serving_input,
                             'documents': self.models[0].serving_input,
        ...
        while not mon_sess.should_stop():
            ...
            try:
                ...
                if task_id == 0 or task_id == 1:
                    # eval, chief worker
                    feed_dict.update({self.is_training: False, self.use_tfrecord: False, self.share_context: False})
                    ...
                    feed_dict.update({self.is_training: True, self.use_tfrecord: False, self.share_context: False})
        ...
        if int(task_id) == 0:  # examples not match
            ...
            while True:
                ...
                if self.train_config.worker_num == len(set(self.finishedWorkers())) + 1:
                    ...
                    # gstep = mon_sess.run(_get_or_create_global_step_read())
                    # self.scaffold.saver.save(mon_sess._coordinated_creator.tf_sess, self.train_config.save_dir + 'sm.ckpt', gstep)
                    ts = self.export_model_vec.export_sm(mon_sess._coordinated_creator.tf_sess)
