...
class KonduModel(ESMM):
    ...
    def forward(self, features, fg):
        ...
        with tf.variable_scope(name_or_scope="%s-Param-Space" % self.name,
            ...
            input_vec_ctr = input_vec
            input_vec_cvr = input_vec
            input_vec_ctvr = input_vec
            with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
                for layer_id, num_hidden_units in enumerate(self.config.dnn_hidden_units):
                    with variable_scope.variable_scope("ctr_hiddenlayer_%d" % layer_id) as hidden_layer_scope:
                        input_vec_ctr = layers.fully_connected(
                            input_vec_ctr,
                            num_hidden_units,
                    ...
                    with variable_scope.variable_scope("cvr_hiddenlayer_%d" % layer_id) as hidden_layer_scope:
                        input_vec_cvr = layers.fully_connected(
                            input_vec_cvr,
                            num_hidden_units,
                            activation_fn=utils.getActivationFunctionOp(self.config.activation_op)
                            if layer_id + 1 < len(self.config.dnn_hidden_units) else None,
                    ...
                    with variable_scope.variable_scope("ctvr_hiddenlayer_%d" % layer_id) as hidden_layer_scope:
                        input_vec_ctvr = layers.fully_connected(
                            input_vec_ctvr,
                            num_hidden_units,
                            activation_fn=utils.getActivationFunctionOp(self.config.activation_op)
                            if layer_id + 1 < len(self.config.dnn_hidden_units) else None,
                ...
                output_vec_ctr = layers.fully_connected(
                    input_vec_ctr,
                    1,
                    activation_fn=None,
                    scope="ctr_ouput_linear_layer",
                    variables_collections=[self.collections_dnn_hidden_layer],
                    outputs_collections=[self.collections_dnn_hidden_output],
                )
                output_vec_cvr = layers.fully_connected(
                    input_vec_cvr,
                    1,
                    activation_fn=None,
                    scope="cvr_ouput_linear_layer",
                    variables_collections=[self.collections_dnn_hidden_layer],
                    outputs_collections=[self.collections_dnn_hidden_output],
                )
                output_vec_ctvr = layers.fully_connected(
                    input_vec_ctvr,
                    1,
                    activation_fn=None,
                    scope="ctvr_ouput_linear_layer",
                    variables_collections=[self.collections_dnn_hidden_layer],
                    outputs_collections=[self.collections_dnn_hidden_output],
                )
        ...
        with tf.variable_scope(name_or_scope="%s-Logits" % self.name,
            ...
            ctr_logits = output_vec_ctr
            cvr_logits = output_vec_cvr
            ctcvr_logits = output_vec_ctvr
    ...
    def trace_op(self):
        ...
        for x in [self.user_input_layer, self.item_input_layer]:
            vstr = tf.as_string(x)
