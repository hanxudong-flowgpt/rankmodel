...
init_assignment_map = {
    ...
    , "local_param_table/queryseg_norm_d_shared_embedding/": "local_param_table/local_key_query_shared_embedding/"
    #, "Share/": "Share/"
...
class KonduModel(SeqAttenModel):
    ...
    def forward(self, features, fg):
        ...
        local_column = fg.feature_columns_from_name(mc.column_groups["local_key_columns"])
        logging.info("start local_column...")
        logging.info(local_column)
        with tf.variable_scope(
                name_or_scope="local_param_table",
                partitioner=base_ops.partitioner(self.config.ps_num,
                                                 mem=8 * 1024 * 1024), reuse=tf.AUTO_REUSE) as scope:
            local_layer = layers.input_from_feature_columns(features, local_column, scope=scope)
        ...
        with tf.variable_scope(name_or_scope="%s-User-Vec-Space" % ("Share" if self.config.share_user_vec else self.name),
            ...
            with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
                ...
                user_norm = tf.nn.l2_normalize(user_vec * local_layer, dim=1)
    ...
    def trace_op(self):
        ...
        if self.config.trace_ivec == "on":
            self.trace["item_vec"] = [self.features["item_id_d"], self.s_ivec_norm]
        if self.config.trace_uvec == "on":
            self.trace["user_vec"] = [self.features["item_id_d"], tf.concat([self.s_uvec_norm, self.position_logits], axis=1)]
        if self.config.trace_uvec != "on" and self.config.trace_ivec != "on":
            self.trace["user_vec"] = [self.features["item_id_d"], self.s_uvec_norm]
