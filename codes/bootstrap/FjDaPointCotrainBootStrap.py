...
import numpy as np
from model.domain.da_pv_cxr_mmd import KonduModel as Model
...
class Train(BootStrap):
    ...
    def _sample_itemfeature_wholestr(self, feature_table, itemlist, mode):
        feature_list = []
        item_mask_list = []
        for itemid in itemlist:
        ...
        return feature_list, item_mask_list
    ...
    def buildGraph(self, task_id, context):
        ...
        for k, v in origin_features.items():
            ...
            unlabel_itemlist = tf.decode_csv(batch_data_list[6], record_defaults=[[""]] * 20, field_delim=",")
            # print(itemlist)
            tablename = "kd_item_features_fj" if self.train_config.is_local else "target_item_feature"

            fea, item_mask_list = self._sample_itemfeature(feature_table, itemlist, tablename, fgconf_map, "train" if self.train_config.task_id > 0 else "eval")
            unlabel_fea, unlabel_item_mask_list = self._sample_itemfeature(feature_table, unlabel_itemlist, tablename, fgconf_map, "train" if self.train_config.task_id > 0 else "eval")
            ...
            features['ublabel_item_feature_map_list'] = unlabel_fea
            features['ublabel_item_feature_map'] = self.convert_feature_map(unlabel_fea, fgconf_map)
            features['ublabel_item_id_list'] = unlabel_itemlist
            features['ublabel_item_mask_list'] = unlabel_item_mask_list
    ...
    def runSession(self, mon_sess, task_id, thread_ind):
        ...
        if not self.any_worker_finished():
            ...
            logging.info("some worker has finished ! skip load feature to ps !")
        ...
        if self.train_config.predict:
            ...
            if self.train_config.task_id in (0, 1):
    ...
    def runTrain(self, mon_sess, task_id, thread_ind):
        ...
        while not mon_sess.should_stop():
            ...
            try:
                ...
                # TODO summary write
                if taskcnt[index] % self.train_config.save_summaries_steps == 0 and task_id in (0, 1):
