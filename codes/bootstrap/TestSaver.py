...
class Train(BootStrap):
    ...
    def buildGraph(self, task_id, context):
        ...
        for k, v in origin_features.items():
            ...
            if self.train_config.platform != 'pai':
                fea, item_mask_list = self._sample_itemfeature(feature_table, itemlist, tablename, fgconf_map,
                ...
                item_feature_list, item_mask_list = self._sample_itemfeature_wholestr(feature_table, itemlist, "train" if self.train_config.task_id > 0 else "eval")
                item_posfeat_list = tf.decode_csv(batch_data_list[7], record_defaults=[[""]] * 10, field_delim=",")
                item_feature_concat = tf.concat(item_feature_list, axis=0)
                item_posfeat_concat = tf.concat(item_posfeat_list, axis=0)

                features['feature_map'] = rtp_fg.parse_genreated_fg(fgconf_list, tf.string_join([item_feature_concat, item_posfeat_concat], separator="\x02"))
                features['feature_map_list'] = None
                # features['feature_map'] = self.convert_feature_map(fea, fgconf_map)
