...
class KonduModel(DaModelSeqAtten):
    ...
    def build_model(self):
        ...
        self.user_vec, self.user_input_layer = self.build_common_user_vec(self.common_features, self.fg)
        ...
        self.s_ivec_norm = []
        self.s_pos_logits = []
        self.s_item_vecs_map_list = []
        self.final_logit_source =[]
        for features in self.source_features:
            ivec_norm, item_vecs_map, ui_input_layer = self.build_item_vec(features, self.fg)
            self.s_ivec_norm.append(ivec_norm)
            self.s_item_vecs_map_list.append(item_vecs_map)
            deep_logits = self.build_deep_logits(self.user_vec, ivec_norm)
            pos_logits = self.build_position_logits(self.user_input_layer, ui_input_layer)
            self.source_logits.append(deep_logits)
            self.s_pos_logits.append(pos_logits)
            self.final_logit_source.append(pos_logits + deep_logits)
        print("%s source model build done..." % self.name)

        print("%s target model build begin..." % self.name)
        self.target_logits = []
        self.t_ivec_norm = []
        self.t_pos_logits = []
        self.t_item_vecs_map_list = []
        self.final_logit_target = []
        for features in self.target_features:
            ivec_norm, item_vecs_map, ui_input_layer = self.build_item_vec(features, self.fg)
            self.t_ivec_norm.append(ivec_norm)
            self.t_item_vecs_map_list.append(item_vecs_map)
            deep_logits = self.build_deep_logits(self.user_vec, ivec_norm)
            pos_logits = self.build_position_logits(self.user_input_layer, ui_input_layer)
            self.target_logits.append(deep_logits)
            self.t_pos_logits.append(pos_logits)
            self.final_logit_target.append(pos_logits + deep_logits)
        print("%s target model build done..." % self.name)
        ...
        smap = {}
        tmap = {}
        for s_item_vecs_map, t_item_vecs_map in zip(self.s_item_vecs_map_list, self.t_item_vecs_map_list):
            for k, v in s_item_vecs_map.items():
                if k in smap:
                    smap[k].append(v)
                    tmap[k].append(t_item_vecs_map[k])
                else:
                    smap[k] = [v]
                    tmap[k] = [t_item_vecs_map[k]]
        self.s_item_vecs_map = {}
        for k, v in smap.items():
            self.s_item_vecs_map[k] = tf.concat(v, axis=0)
        self.t_item_vecs_map = {}
        for k, v in tmap.items():
            self.t_item_vecs_map[k] = tf.concat(v, axis=0)

        self.mmdloss = {}
        for k, v in self.s_item_vecs_map.items():
            self.mmdloss[k] = mmd.mmd_loss(v, self.t_item_vecs_map[k])

    def build_common_user_vec(self, features, fg):
        mc = self.mc
        ...
        '''
        if not (user_column and query_column):
    ...
    def build_item_vec(self, features, fg):
        # logging.info(item_column)
        item_column = fg.feature_columns_from_name(self.mc.item_columns)
        ui_column = fg.feature_columns_from_name(self.mc.ui_columns)
        with tf.variable_scope(
                name_or_scope="input_from_feature_columns",
                partitioner=base_ops.partitioner(self.config.ps_num,
                                                 mem=8 * 1024 * 1024), reuse=tf.AUTO_REUSE) as scope:
            item_input_layer = layers.input_from_feature_columns(features, item_column, scope=scope)
        ...
        return item_norm, item_vecs_da, ui_input_layer
    ...
    def trace_op(self):
        ...
        if self.config.trace_ivec == "on":
            self.trace["item_vec"] = [self.source_features[0]["item_id_d"], self.s_ivec_norm[0]]
        if self.config.trace_uvec == "on":
            self.trace["user_vec"] = [self.source_features[0]["item_id_d"],
                                      tf.concat([self.user_vec] + self.s_pos_logits, axis=1)]
        if self.config.trace_uvec != "on" and self.config.trace_ivec != "on":
            self.trace["user_vec"] = [self.source_features[0]["item_id_d"], self.user_vec]
        if self.training_config.platform == 'pai' and self.training_config.predict:
            ...
            predict_show = []
            predict_show.append(self.source_id[0])
            predict_show.append(tf.as_string(self.source_predictions[0]))
            vstr = tf.as_string(self.trace["item_vec" if self.config.trace_ivec == "on" else "user_vec"][-1])
    ...
    def loss_op(self, task_label=None, task_num=None):
        with tf.name_scope("Loss_Op"):
            if self.name == "Recall_weighted":
                self.source_loss = tf.reduce_mean(
                    tf.nn.weighted_cross_entropy_with_logits(
                        targets=tf.concat(self.source_label, axis=0),
                        logits=tf.concat(self.source_logits, axis=0) + tf.concat(self.s_pos_logits, axis=0),
                        pos_weight=tf.concat(self.source_weight, axis=0)))
            else:
                self.source_loss = tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(
                        logits=tf.concat(self.source_logits, axis=0) + tf.concat(self.s_pos_logits, axis=0),
                        labels=tf.concat(self.source_label, axis=0)))

            # target entropy
            trg_sig = tf.maximum(tf.sigmoid(tf.concat(self.target_logits, axis=0)) * 30.0, 1e-16)
            trg_sig = tf.minimum(trg_sig, 0.999)
            self.target_loss = - tf.reduce_mean(trg_sig * tf.log(trg_sig) + (1 - trg_sig) * tf.log(1 - trg_sig))

            # Set up the training ops.
            self.reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
            reg_losses_model = self.reg_losses[
                                    int((1.0 - 1.0 / (float(self.model_idx) + 1.0)) * len(self.reg_losses)):]

            # logging.info(self.name + "-" + str(self.task_id) + "-" + str(len(self.reg_losses_model)))
            # import sys
            # sys.stdout.flush()

            self.reg_losses = tf.reduce_sum(reg_losses_model)
            self.loss = self.reg_losses + self.source_loss
            if self.config.target_entropy_loss_weight != 0.0:
                self.loss += self.config.target_entropy_loss_weight * self.target_loss
            if self.config.mmd_loss_weight != 0.0:
                # self.loss += self.config.mmd_loss_weight * self.mmdloss["item_vec_fc0_out"]
                self.loss += self.config.mmd_loss_weight * self.mmdloss["item_vec_fc1_out"]
        return self.loss
    ...
    def summary(self):
        ...
        with tf.name_scope("%s-Auxi-Loss" % self.name):
            ...
            tf.summary.scalar("target_entropy_loss", self.target_loss)
            tf.summary.scalar("reg_losses", self.reg_losses)

        with tf.name_scope("%s-MMD-Loss" % self.name):
            for k, v in self.mmdloss.items():
                tf.summary.scalar(k, v)

        with tf.name_scope("%s-SourceDomain" % self.name):
            for k, v in self.s_item_vecs_map.items():
        ...
        with tf.name_scope("%s-TargetDomain" % self.name):
            for k, v in self.t_item_vecs_map.items():
                tf.summary.histogram(k, v)
                shape = v.get_shape().as_list()
                mean, var = tf.nn.moments(v, range(len(shape)))
                tf.summary.scalar(k + "_mean", tf.squeeze(mean))
