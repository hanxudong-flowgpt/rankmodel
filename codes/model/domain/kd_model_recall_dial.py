...
class KonduModel(DaModelSeqAtten):
    ...
    def build_model(self):
        ...
        self.source_logits, self.s_pos_logits, self.s_uvec_norm, self.s_ivec_norm, self.s_item_vecs_map = \
            self.forward(self.source_features, self.fg)
        print("%s source model build done..." % self.name)
        self.target_logits, self.t_pos_logits, self.t_uvec_norm, self.t_ivec_norm, self.t_item_vecs_map = \
            self.forward(self.target_features, self.fg)
        ...
        self.final_logit_source = self.source_logits + self.s_pos_logits
        self.final_logit_target = self.target_logits + self.t_pos_logits
    ...
    def trace_op(self):
        ...
        if self.config.trace_ivec == "on":
            self.trace["item_vec"] = [self.source_features["item_id_d"], self.s_ivec_norm]
        if self.config.trace_uvec == "on":
            self.trace["user_vec"] = [self.source_features["item_id_d"], tf.concat([self.s_uvec_norm, self.s_pos_logits], axis=1)]
        if self.config.trace_uvec != "on" and self.config.trace_ivec != "on":
            self.trace["user_vec"] = [self.source_features["item_id_d"], self.s_uvec_norm]
        if self.training_config.platform == 'pai' and self.training_config.predict:
            self.odpsWriter = tf.TableRecordWriter(tf.flags.FLAGS.tableoutput, slice_id=self.task_id)
            self.odpsWriterClose = self.odpsWriter.close()
            predict_show = []
            predict_show.append(self.source_id)
            predict_show.append(tf.as_string(self.source_predictions))
    ...
    def loss_op(self, task_label=None, task_num=None):
        with tf.name_scope("Loss_Op"):
            if self.name == "Recall_weighted":
                self.source_loss = tf.reduce_mean(
                    tf.nn.weighted_cross_entropy_with_logits(
                        targets=self.source_label,
                        logits=self.source_logits + self.s_pos_logits,
                        pos_weight=self.source_weight))
            else:
                self.source_loss = tf.reduce_mean(
            ...
            # target entropy
            trg_sig = tf.maximum(tf.sigmoid(self.target_logits) * 30.0, 1e-16)
