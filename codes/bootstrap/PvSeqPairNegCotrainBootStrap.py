...
class Train(BootStrap):
    ...
    def buildGraph(self, task_id, context):
        ...
        for modelconf in jobconf["sources"]:
            ...
            self.models.append(Model(self.train_config, model_config, mc, modelconf["model_name"], idx, 30))
        ...
        for k, v in origin_features.items():
            ...
            itemlist = [tf.decode_csv(xx, record_defaults=[[""]] * 5, field_delim="_")[0] for xx in xxlist]
            neg_list = tf.decode_csv(batch_data_list[6], record_defaults=[[""]] * 20, field_delim=",")
            itemlist.extend(neg_list)
            print(itemlist)
            ...
            features['label_list'].extend([0 * features['label_list'][0]] * 20)
