...
class KonduModel(CxrModel):
    ...
    def forward(self, features, fg):
        ...
        if mc.sequence_columns:
            logging.info("start user_seq_layer...")
            if self.config.share_user_layer:
                items_stack = self.user_seq_layer(user_input_layer, query_input_layer, mc.user_seq_columns, fg, features)
            else:
                items_stack = self.user_seq_layer(user_input_layer, query_input_layer, mc.user_seq_columns, fg, features,
                                                  self.name)
            user_input_layer = tf.concat(values=[user_input_layer, items_stack], axis=1)
        if mc.fav_seq_column_dict.keys():
            logging.info("start fav_seq_column_dict...")
            seqlen = mc.config_dict['fav_seq_columns'][0]['sequence_length']
            with variable_scope.variable_scope("fav_seq_columns"):
                items_stack = self.user_seq_layer(user_input_layer, query_input_layer, mc.fav_seq_column_dict.keys(),
                                                  features, seqlen, None if self.config.share_user_layer else self.name)
            user_input_layer = tf.concat(values=[user_input_layer, items_stack], axis=1)
        if mc.buy_seq_column_dict.keys():
            logging.info("start buy_seq_column_dict...")
            seqlen = mc.config_dict['buy_seq_columns'][0]['sequence_length']
            with variable_scope.variable_scope("buy_seq_columns"):
                items_stack = self.user_seq_layer(user_input_layer, query_input_layer, mc.buy_seq_column_dict.keys(),
                                                  features, seqlen, None if self.config.share_user_layer else self.name)
            user_input_layer = tf.concat(values=[user_input_layer, items_stack], axis=1)
        if mc.clk_seq_column_dict.keys():
            logging.info("start clk_seq_column_dict...")
            seqlen = mc.config_dict['clk_seq_columns'][0]['sequence_length']
            with variable_scope.variable_scope("clk_seq_columns"):
                items_stack = self.user_seq_layer(user_input_layer, query_input_layer, mc.clk_seq_column_dict.keys(),
                                                  features, seqlen, None if self.config.share_user_layer else self.name)
            user_input_layer = tf.concat(values=[user_input_layer, items_stack], axis=1)
