...
class KonduModel(SeqAttenModel):
    ...
    def forward(self, features, fg):
        ...
        cross_column = fg.feature_columns_from_name(mc.cross_columns)
        ...
        with tf.variable_scope(
            ...
            logging.info(query_input_layer)
            ...
            logging.info(item_input_layer)
            # logging.info(cross_column)
            if cross_column:
                logging.info("start cross_input_layer...")
                cross_input_layer = layers.input_from_feature_columns(features, cross_column, scope=scope)
                logging.info(cross_input_layer)
            if ui_column:
        ...
        with tf.variable_scope(name_or_scope="%s-Param-Space" % self.name,
            ...
            inputs = [user_input_layer, item_input_layer, cross_input_layer] if cross_column else [user_input_layer, item_input_layer]
