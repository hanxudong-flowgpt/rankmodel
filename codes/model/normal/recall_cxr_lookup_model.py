...
class KonduModel(SeqAttenModel):
    ...
    def forward(self, features, fg):
        ...
        lookup_column = fg.feature_columns_from_name(mc.column_groups["lookup_columns"])
        with tf.variable_scope(
                name_or_scope="lookup_features",
                partitioner=base_ops.partitioner(self.config.ps_num,
                                                 mem=8 * 1024 * 1024), reuse=tf.AUTO_REUSE) as scope:
            lookup_layer = layers.input_from_feature_columns(features, lookup_column, scope=scope)
        ...
        with tf.variable_scope(name_or_scope="%s-Logits" % self.name,
            ...
            deep_logits += tf.reduce_sum(lookup_layer, axis=1, keep_dims=True)
