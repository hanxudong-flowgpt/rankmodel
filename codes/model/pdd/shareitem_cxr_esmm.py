...
class KonduModel(ESMM):
    ...
    def get_user_vec(self, user_input_layer, batch_size_tensor, scope_name):
    ...
    def forward(self, features, fg):
        ...
        uvec_ctr = self.get_user_vec(user_input_layer, tf.shape(item_input_layer)[0], "uvec_ctr")
        uvec_cvr = self.get_user_vec(user_input_layer, tf.shape(item_input_layer)[0], "uvec_cvr")
        uvec_ctcvr = self.get_user_vec(user_input_layer, tf.shape(item_input_layer)[0], "uvec_ctcvr")

        if 'item_vecs' in features and 'item_vecs_da' in features:
            item_norm = features['item_vecs']
            item_vecs_da = features['item_vecs_da']
        else:
        ...
        with tf.variable_scope(name_or_scope="%s-Logits" % self.name,
            ...
            ctr_logits = tf.reduce_sum(tf.multiply(uvec_ctr, item_norm), axis=1, keepdims=True)
            cvr_logits = tf.reduce_sum(tf.multiply(uvec_cvr, item_norm), axis=1, keepdims=True)
            ctcvr_logits = tf.reduce_sum(tf.multiply(uvec_ctcvr, item_norm), axis=1, keepdims=True)
            deep_logits = [ctr_logits, cvr_logits, ctcvr_logits]
            pos_logits = 0.0
            '''
        ...
        #return deep_logits, pos_logits, uvec_ctr, item_norm, item_vecs_da
        return deep_logits, pos_logits, uvec_ctr, item_norm, item_vecs_da
    ...
    def trace_op(self):
        ...
        predict_show = []
        predict_show.append(self.id)
        predict_show.append(tf.as_string(self.label[0]))
        predict_show.append(tf.as_string(self.predictions[0]))
        for x in [self.s_uvec_norm, self.user_input_layer, self.item_input_layer, self.s_ivec_norm]:
            vstr = tf.as_string(x)
    ...
    def loss_op(self, task_label=None, task_num=None):
        with tf.name_scope("Loss_Op"):
            ctr_label = tf.cast(tf.not_equal(self.label, 0.0), tf.float32)
            ctcvr_label = tf.cast(tf.equal(self.label, 5.0), tf.float32)
            self.label = [ctr_label, ctcvr_label, ctcvr_label]
            self.auc_weights = [1.0, ctr_label, 1.0]
            self.celoss_ctr = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=self.final_logits[0],
                    labels=ctr_label))
            self.celoss_cvr = tf.reduce_mean(ctr_label *
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=self.final_logits[1],
                    labels=ctcvr_label))
            self.celoss_ctcvr = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=self.final_logits[2],
                    labels=ctcvr_label))

            # Set up the training ops.
            self.reg_losses = self.reg_loss_op()
            self.loss = self.reg_losses + self.celoss_ctr + self.celoss_cvr + self.celoss_ctcvr
        return self.loss
    ...
    def summary(self):
        ...
        with tf.name_scope("%s-Auxi-Loss" % self.name):
            ...
            tf.summary.scalar("ctr_loss", self.celoss_ctr)
            tf.summary.scalar("cvr_loss", self.celoss_cvr)
            tf.summary.scalar("ctcvr_loss", self.celoss_ctcvr)
