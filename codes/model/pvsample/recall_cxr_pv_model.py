...
from model.pvsample.seq_pv_model import SeqAttenModel
...
class KonduModel(SeqAttenModel):
    def __init__(self,
        ...
        self.cross_entropy_logits_list = []
        self.ivec_norm = []
        self.position_logits_list = []
        self.final_logits_list = []
    ...
    def build_model(self):
        ...
        for features in self.feature_map_list:
            ...
            self.cross_entropy_logits_list.append(deep_logits)
            self.position_logits_list.append(pos_logits)
            self.final_logits_list.append(pos_logits + deep_logits)
    ...
    def trace_op(self):
        ...
        if self.config.trace_ivec == "on":
            self.trace["item_vec"] = [self.common_features["item_id_d"], self.ivec_norm]
        if self.config.trace_uvec == "on":
            self.trace["user_vec"] = [self.common_features["item_id_d"], tf.concat([self.user_vec] + self.position_logits_list, axis=1)]
        if self.config.trace_uvec != "on" and self.config.trace_ivec != "on":
            self.trace["user_vec"] = [self.common_features["item_id_d"], self.user_vec]
            ...
            if self.name == "Recall_weighted":
                self.celoss = tf.reduce_mean(
                    tf.nn.weighted_cross_entropy_with_logits(
                        targets=tf.concat(self.label_list, axis=0),
                        logits=tf.concat(self.cross_entropy_logits_list, axis=0) + tf.concat(self.position_logits_list, axis=0),
                        pos_weight=tf.concat(self.weight_list, axis=0)))
            else:
                self.celoss = tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(
                        logits=tf.concat(self.cross_entropy_logits_list, axis=0) + tf.concat(self.position_logits_list, axis=0),
                        labels=tf.concat(self.label_list, axis=0)))

            # Set up the training ops.

            self.reg_losses = self.reg_loss_op()
            self.loss = self.reg_losses + self.celoss
            self.loss = tf.Print(self.loss, [x if isinstance(x, tf.Tensor) else x.values for x in self.feature_map_list[0].values()])
        return self.loss

    def predictions_op(self):
        self.predictions_list = [tf.sigmoid(x) for x in self.final_logits_list]
        self.predictions = tf.concat(self.predictions_list, axis=0)
        self.out4rtp["uservec"] = tf.reduce_mean(self.user_vec, axis=1) + self.position_logits
