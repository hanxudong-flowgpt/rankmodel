...
import traceback
from feature.sample_io import build_hdfs_inputs_iterator
...
class Train(BootStrap):
    ...
    def buildGraph(self, task_id, context):
        ...
        for modelconf in jobconf["sources"]:
            ...
            fn_var = tf.placeholder_with_default()
            self.file_name_phs.append(fn_var)
            model_file_queue = data_flow_ops.FIFOQueue(capacity=1, dtypes=tf.string, shapes=(),
                                                       shared_name='%s_file_name_queue' % model_config.model_name)
            self.file_queues.append(model_file_queue)
            self.file_enqueue_ops.append(model_file_queue.enqueue(fn_var))
            file_list = tf.data.ReadQueueDataset(model_file_queue)
            bs = 10 if self.train_config.is_local else self.train_config.batch_size
            ds = tf.data.TextLineDataset(file_list,
                                         compression_type='GZIP' if self.train_config.data_format == 'gz' else None,
                                         buffer_size=8 * 1024 * 1024).batch(bs).prefetch(bs * 10)
            itera = ds.make_initializable_iterator()
    ...
    def createSession(self, target, task_id):
        ...
        if self.train_config.platform != "local":
            ...
            return tf.train.MonitoredTrainingSession(
                ...
                # config=tfconf,
                save_summaries_steps=self.train_config.save_summaries_steps if task_id < 1 else None,
                # save_summaries_secs=None,
                summary_dir=logdir,
                hooks=None,
                scaffold=tf.train.Scaffold(local_init_op=tf.group(tf.local_variables_initializer(), [x.initializer for x in self.data_iterators])),
    ...
    def runSession(self, mon_sess, task_id, thread_ind):
        if self.train_config.predict:
            ...
            if task_id == 0:
                ...
                self._saved_model_dir = self.train_config.save_dir + 'saved_model/'
                mon_sess.graph._unsafe_unfinalize()
                timestamp_dir = str(int(current_timestamp))
                saved_model_dir = self._saved_model_dir + '/' + timestamp_dir
                logging.info('starting to export saved model to %s' % saved_model_dir)
                simple_save(mon_sess._coordinated_creator.tf_sess,
                            saved_model_dir, saved_model_input, saved_model_output)
                mon_sess.graph.finalize()
                done_list = self._saved_model_dir + '/done_list'
                if file_io.file_exists(done_list):
                    done_list_content = file_io.read_file_to_string(done_list)
                    done_list_content += '\n' + timestamp_dir
                else:
                    done_list_content = timestamp_dir
                file_io.write_string_to_file(done_list, done_list_content)
            logging.info("finish train")
