...
class KonduModel(CxrModel):
    ...
    def forward(self, features, fg):
        ...
        with tf.variable_scope(name_or_scope="%s-Logits" % self.name,
            ...
            if self.config.ip_use_constant:
                ip_const = self.config.ip_constant
            else:
                with tf.variable_scope(name_or_scope="%s-inner-product" % self.name,
                                       reuse=tf.AUTO_REUSE):
                    ip_const = tf.Variable(initial_value=self.config.ip_constant, name="%s-ip-factor" % self.name)
            # deep_logits = tf.multiply(deep_logits, paddings * (ip_const))
    ...
    def trace_op(self):
        ...
        v = tf.reduce_join(vstr, 1, keep_dims=True, separator=',')
        self.pk = tf.sparse_tensor_to_dense(self.features['7'], default_value="0")
        logging.info("all feature in model:")
        logging.info(self.features)
        for x in self.training_config.vec_append_feature:
            if x in self.features:
                self.vec_append_fea.append(tf.sparse_tensor_to_dense(self.features[x], default_value="0"))
