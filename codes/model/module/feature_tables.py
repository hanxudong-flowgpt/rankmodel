...
class FeatureTableReadyHook(session_run_hook.SessionRunHook):
  ...
  def __init__(self, load_ops, after_load_op, after_ready_op,
               all_worker_status, ready_op, check_wait_secs, table_stats, readynum, worker_id, data):
    self._load_ops = load_ops
    self._after_load_op = after_load_op
    # self._ever_loaded_op = ever_loaded_op
    self._all_worker_status = all_worker_status
    self._ready_op = ready_op
    self._check_wait_secs = check_wait_secs
    self._hash_table_state = table_stats
    self._after_ready_op = after_ready_op
    self._ready_num = readynum
  ...
  def after_create_session(self, session, coord):
    ...
    time.sleep(self._check_wait_secs * 2)
  ...
  def _wait_for_ready(self, session):
    ...
    while True:
      ...
      if ready:
        ...
        all_worker_status = session.run(self._all_worker_status)
        worker_not_ready = []
        ...
        table_stat = session.run(self._hash_table_state)
        logging.info(
...
class FeatureTable(object):
  ...
  def __init__(self, input_tables_conf, server_count=None,
          worker_index=None, worker_count=None, check_wait_secs=15):
