...
from feature.input_from_fc import dense_repeat, input_from_feature_columns
...
class KonduModel(SeqAttenModel):
    ...
    def forward(self, features, fg):
        ...
        with tf.variable_scope(
            ...
            item_input_layer = input_from_feature_columns(features, item_column,
                                                          use_fast_embeding=self.training_config.train_parts[0] == 'pms',
                                                          share_context=self.training_config.share_context, scope=scope)

            logging.info("start user_input_layer...")
            user_input_layer = input_from_feature_columns(features, user_column,
                                                          use_fast_embeding=self.training_config.train_parts[0] == 'pms',
                                                          share_context=self.training_config.share_context, scope=scope)
            '''
            user_input_layer = export_trick.input_from_feature_columns_with_repeating(features, user_column,
                                                                                                repeating_columns=user_column,
                                                                                                n=batch_size,
                                                                                                share_context=self.training_config.share_context,
                                                                                                scope=scope)
            '''
            logging.info("start query_input_layer...")
            query_input_layer = input_from_feature_columns(features, query_column,
                                                           use_fast_embeding=self.training_config.train_parts[0] == 'pms',
                                                           share_context=self.training_config.share_context, scope=scope)
            '''
            query_input_layer = export_trick.input_from_feature_columns_with_repeating(features, query_column,
                                                                                       repeating_columns=query_column,
                                                                                       n=batch_size,
                                                                                       share_context=self.training_config.share_context,
                                                                                       scope=scope)
            '''
            if ui_column:
                ...
                ui_input_layer = input_from_feature_columns(features, ui_column,
                                                            use_fast_embeding=self.training_config.train_parts[0] == 'pms',
                                                            share_context=self.training_config.share_context, scope=scope)
            else:
                ui_input_layer = None
        ...
        if self.training_config.version == "0.1":
    ...
    def trace_op(self):
        ...
        vec_key = tf.sparse.to_dense(self.features['7'], default_value="0")
        self.predict_vector = tf.reduce_join(tf.string_join([vec_key, v], "\t"),
                                             axis=None,
                                             keep_dims=False,
                                             separator="\n")
