...
class KonduModel(SeqAttenModel):
    def __init__(self,
        ...
        super(KonduModel, self).__init__(train_config, model_config, model_columns, model_name, model_idx, item_num_in_pv)
    ...
    def build_model(self):
        ...
        with tf.variable_scope(
            ...
            if ui_column:
                ...
                self.first_ui_input_layer = layers.input_from_feature_columns(self.common_features, ui_column, scope=scope)
            else:
                ui_input_layer = None
        ...
        deep_logits, self.ip = self.build_deep_logits(tf.concat([self.user_vec] * self.item_num_in_pv, axis=0), self.ivec_norm)
        pos_logits = self.build_position_logits(tf.concat([self.user_input_layer] * self.item_num_in_pv, axis=0), self.ui_input_layer)
        self.first_position_logits = self.build_position_logits(self.user_input_layer, self.first_ui_input_layer)
        self.cross_entropy_logits = deep_logits
        self.position_logits = pos_logits
        self.final_logits = pos_logits + deep_logits if self.model_config.usepos else deep_logits
    ...
    def trace_op(self):
        ...
        self.trace["user_vec"] = tf.concat([tf.concat([self.user_vec] * self.item_num_in_pv, axis=0), self.position_logits], axis=1)
    ...
    def predictions_op(self):
        ...
        if self.model_config.usepos:
            self.out4rtp["uservec"] = tf.reduce_mean(self.user_vec, axis=1) + self.first_position_logits
        else:
