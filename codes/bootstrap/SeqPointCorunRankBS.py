...
import numpy as np
from model.pdd.rank_cxr_model import KonduModel as Model
...
class Train(BootStrap):
    ...
    def buildGraph(self, task_id, context):
        ...
        '''
        def switch_data(start, models, iteras, which_model):
            assert len(models) == len(iteras)
            if len(models) > 2:
                x = tf.equal(which_model, tf.constant(start))
                return tf.cond(x, lambda: iteras[0].get_next(),
                               lambda: switch_data(start+1, models[start+1:], iteras[start+1], which_model))
            if len(models) == 2:
                x = tf.equal(which_model, tf.constant(start))
                return tf.cond(x, lambda: iteras[0].get_next(),
                               lambda: iteras[1].get_next())
            elif len(models) == 1:
                return iteras[0].get_next()
            else:
                raise ValueError("switch data error")
        self.batch_data = switch_data(0, self.models, self.train_itera, self.which_model)
        ...
        # self.parsed_feature, self.final_features = self.fg.build_tf_tensors_share_context(self.batch_data, self.use_tfrecord, self.share_context, self.context_serving_ph)
    ...
    def createSession(self, target, task_id):
        ...
        saved_model_input = {'context': self.context_serving_ph,
                             ...
                             # 'vector': self.models[0].s_ivec_norm
            ...
            try:
                ...
                if task_id == 0 or task_id == 1:
                    ...
                    co_global_step, global_step, _, loss, auc, totalauc, flocalv = mon_sess.run(
                        [self.global_step, model.global_step, train_op, model.loss, model.metrics['scalar/batch_auc'],
                         model.metrics['scalar/total_auc'], localvar],
                        feed_dict=feed_dict)
                    # co_global_step, global_step = steps
                    # print(seeds)
                    logging.info('%s-Col_Global_Step:%s, Global_Step:%s, loss=%s, totalauc=%s task_id=%s thread=%s' % (
                        model.model_name, str(co_global_step), str(global_step), str(loss),
                        str(totalauc), str(task_id), str(thread_ind)))
        ...
        if int(task_id) == 0:  # examples not match
            ...
            while True:
                ...
                if self.train_config.worker_num == len(set(self.finishedWorkers())) + 1:
                    ...
                    if self.train_config.train_parts[0] == 'pms':
