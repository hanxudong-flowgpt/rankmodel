...
input_params = {
    'pool_table_selected_columns': 'item_id,vec,querys',
    'input_table_selected_columns': 'item_id,vec,query'
}
...
class TopkRecaller(object):
    ...
    def init_embedding(self, worker_num, worker_id):
        ...
        if worker_id == worker_num-1:
            ...
            inquerys = list(self.querys)[start:]
        else:
            start = worker_id * x
            end = worker_id * x + x
            item_vecs = list(self.item_vec)[start:end]
            item_ids = list(self.item_id)[start:end]
            inquerys = list(self.querys)[start:end]
        logging.info("worker id:%d, init embeddings start:%d, end:%d" % (worker_id, start, start+x))
        assign_array = []
        for i in range(len(item_vecs)):
            item_id_tmp = item_ids[i]
            item_vec_tmp = item_vecs[i]
            query_tmp = inquerys[i]
            ...
            row_size_q = query_tmp.shape[0]
            assert row_size_id == row_size_vec, "vec size not equal to vec size:{}, {}".format(row_size_id, row_size_vec)
            assert row_size_id == row_size_q, "vec size not equal to vec size:{}, {}".format(row_size_id, row_size_q)
            key, record_op = reader.read_up_to(filename_queue, num_records=row_size_id)

            default = [tf.constant(['0'], dtype=tf.string) for i in range(3)]
            ...
            logging.info('item_id:', item_id, 'vec:', vec)
            ...
            assign_array.append(assign_init)
            assign_init = tf.assign(query_tmp, querys)
            assign_array.append(assign_init)
    ...
    def build_input(self):
        with tf.device(self.work_device):
            ...
            default = [tf.constant(['0'], dtype=tf.string) for i in range(3)]
            input_id, input_vec, query = tf.decode_csv(record_op, record_defaults=default, field_delim=":")

            default = [tf.constant([0.0], dtype=tf.float32) for i in range(0, self.item_emb_dim)]
            vec_raw = tf.decode_csv(input_vec, default, field_delim=',')
            input_vec = tf.stack(vec_raw, axis=1)
        return input_id, input_vec, query

    def build_model(self):
        input_id, input_vec, input_query = self.build_input()
        #with tf.device('/gpu:0'):
        item_vec =  tf.get_variable('item_vec', [self.item_emb_num, self.item_emb_dim],
            initializer=tf.zeros_initializer(), partitioner=self.partitioner, trainable=False)
        item_id = tf.get_variable('item_id', [self.item_emb_num], dtype=tf.int64, partitioner=self.partitioner,
                                  initializer=tf.zeros_initializer(), trainable=False)
        querys = tf.get_variable('querys', [self.item_emb_num], dtype=tf.string, partitioner=self.partitioner,
                                 initializer=tf.zeros_initializer(), trainable=False)


        self.item_id = item_id
        self.item_vec = item_vec
        self.querys = querys

        out_layer = [None] * len(list(item_vec))
        for p in range(len(list(item_vec))):
            with ops.colocate_with(list(item_vec)[p]):
                ip = tf.matmul(input_vec, list(self.item_vec)[p], transpose_b=True)
                mask = tf.regex_full_match(list(self.querys)[p], tf.reshape(input_query, []))
                logging.info('input_vec:', input_vec)
                out_layer[p] = (ip + 10.0) * tf.cast(mask, tf.float32)
                # logging.info('item_vec:', list(self.item_vec)[p])
        ...
        logging.info('factor:', factor)

        logging.info('output_layer:', out_layer)
        logging.info('factor:', factor)
    ...
    def get_top_k(slef, value_list, factor, k=50):
        ...
        logging.info(value_list[0].get_shape())
        logging.info(value_list)
        ...
        logging.info('len_of_index_base')
        logging.info(len_of_index_base)
        logging.info(len_of_each_partition)
        logging.info(value_list)

        indices = [x[1] for x in top_k_of_each]
        logging.info('indices before add')
        # logging.info(sess.run([indices]))
        ...
        logging.info("flatten_index:")
        logging.info(flatten_index)
        # logging.info(sess.run(flatten_index))
        logging.info('flatten_new_index')
        logging.info(flatten_new_index)
        # logging.info(sess.run([flatten_new_index]))
        final_index = tf.gather(flatten_index, flatten_new_index)
        final_index = tf.reshape(final_index, [-1, k])
        logging.info('final_index')
        # logging.info(sess.run([final_index]))
        ...
        with tf.train.MonitoredTrainingSession(master=server.target,
            ...
            while not mon_sess.should_stop():
                try:
                    ...
                    logging.info("load all vectors: %s sec" % str(time.time() - t))
                except tf.errors.OutOfRangeError:
                    logging.info('load base item vector done...')
                    logging.info('waiting other workers...')
                    recaller.wait_for_ready(mon_sess)
                    logging.info("all worker load: %s sec" % str(time.time() - t))
                    logging.info('all workers load base item vector done...')
                    break
            while not mon_sess.should_stop():
                ...
                try:
                    for _ in range(5):
                        ...
                        logging.info("recall one batch: %s sec" % str(time.time() - t))
                    break
                except tf.errors.OutOfRangeError:
                    break
            mon_sess.run(recaller.close_table)
            logging.info('get topk item done...')
